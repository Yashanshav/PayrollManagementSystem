...
class SalarySlipFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        var lf = 0;
        if(leaves.leavesTaken > leaves.availableLeaves) lf = leaves.leavesTaken-leaves.availableLeaves
        val tax = calculateTax(salary.basicPay)
        var netTax = 0
        netTax = if(tax >= salary.pf) (tax - salary.pf).roundToInt()
        else (salary.pf - tax).roundToInt()
        val amount = (salary.basicPay + salary.hra + salary.ta + salary.da + salary.otherAllowances)/12 - netTax -((salary.basicPay/30)*lf)
